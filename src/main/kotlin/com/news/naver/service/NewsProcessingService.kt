...
class NewsProcessingService(
    ...
    suspend fun processNews(channel: NewsChannel) = coroutineScope {
        ...
        val lastArticleTime = newsArticleRepository.selectNewsArticleLatest()?.publishedAt ?: LocalDateTime.now().minusDays(1)
        ...
        newsItems.asFlow()
            ...
            .filter { (_, _, _, hash) -> !newsArticleRepository.countNewsArticleByNaverLinkHash(hash) } // 2. DB에 없는 새로운 뉴스만 (해시 기준)
            ...
            .forEach { article ->
                launch {
                    val savedArticle = newsArticleRepository.createNewsArticle(article)
                    ...
                    deliveryLogRepository.createDeliveryLog(
                        DeliveryLogEntity(
