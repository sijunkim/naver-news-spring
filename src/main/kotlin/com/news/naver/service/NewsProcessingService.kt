...
import com.news.naver.repository.NewsArticleCustomRepository
 ...
 * @property newsArticleCustomRepository 뉴스 기사 데이터에 접근하기 위한 커스텀 리포지토리
...
class NewsProcessingService(
    ...
    private val newsArticleCustomRepository: NewsArticleCustomRepository,
    ...
    suspend fun processNews(channel: NewsChannel) = coroutineScope {
        ...
        val lastArticleTime = newsArticleCustomRepository.selectNewsArticleTopByPublishedAtDesc()?.publishedAt ?: LocalDateTime.now().minusDays(1)
        ...
        newsItems.asFlow()
            ...
            .filter { (_, _, _, hash) -> !newsArticleCustomRepository.selectNewsArticleExistsByNaverLinkHash(hash) } // 2. DB에 없는 새로운 뉴스만 (해시 기준)
                ...
                launch {
                    ...
                    deliveryLogRepository.save(
                        DeliveryLogEntity(
                            ...
                            httpStatus = 200,
                            responseBody = MessageConstants.SLACK_SUCCESS_RESPONSE
                        )
                    )
                }
            }
 ...
 */
suspend fun com.news.naver.repository.NewsArticleRepository.findTopByOrderByPublishedAtDesc(): com.news.naver.entity.NewsArticleEntity? = this.findAll().toList().maxByOrNull { it.publishedAt!! }
"Start processing news for channel: ${channel.name}")
